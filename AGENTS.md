...
- When students already know a prerequisite from an earlier course, treat it as assumed background or retrieve it briefly; use the lecture's limited slide time to advance the new mechanism rather than reteaching the prerequisite.
...
- On code-forward slides, group related examples through fragment timing rather than visible group labels, separators, or nested containers. Keep the code surface visually close to a clean editor, with only brief secondary commentary where the instructor will supply the fuller explanation orally.
...
- When introducing a new architecture family, first compare its visible module structure with the previously studied architecture, then isolate the new design principle, and finally show how that structure maps to executable framework code. Keep the architecture visual and code close enough that students can trace modules between them.
- Present a single reference image with the shared centered-figure component rather than placing it arbitrarily in a lecture-local column. Keep the image centered and large, and use the shared caption hierarchy for a readable label, interpretation, prompt, and source; do not substitute a tiny detached source line for a real caption.
...
- Keep citations visually subordinate to slide content but comfortably readable at 1440×900; use approximately 11–13 px for source lines rather than shrinking them into fine print.
- Let boxes grow naturally with their content by default. Use explicit minimum or fixed heights only when a specific visual comparison requires them; resolve crowding through layout, wrapping, and spacing rather than shrinking the text.
...
- Treat `quarto-site/lectures/01-what-is-learning/slides.css` as the shared course component library. Before creating lecture-local boxes, prompts, answer panels, takeaways, diagrams, or exercise elements, reuse an approved shared component when one exists. Lecture CSS may control slide-specific geometry or introduce a genuinely new visual component, but it must not restyle the same semantic element arbitrarily from slide to slide. After the instructor approves a reusable new pattern, promote its base styling to the shared stylesheet and keep only its content-specific variant in the lecture stylesheet.
- Compose slides from the shared components according to the teaching sequence; do not turn a preferred component into a repeated slide template. Vary the composition to fit the idea—such as a full-width diagram, temporary question overlay, staged evidence, comparison, or final takeaway—while preserving the component’s established appearance.
- Preserve an approved component’s visual language without repeating one slide composition mechanically. For API abstractions that students already understand conceptually, foreground the new programming contract with a concise definition and compare a generic template directly with its concrete implementation; omit supporting diagrams when they add little explanatory value.
- Use one stable code font size across all instances of the shared editor component within a lecture. Adjust panel geometry, code length, or line spacing to resolve crowding; do not make code type visibly change size from slide to slide.
- Before presenting a newly authored section for instructor review, render it and visually inspect every slide at 1440×900, including each important fragment state. Check specifically for overflowing or clipped text, Quarto-generated paragraph wrappers that constrain grid content, empty or orphaned boxes during reveals, inconsistent spacing, unreadably small type, and repetitive slide composition. Fix these issues before declaring the section ready.
...
- For CNN architecture figures, use compact layer bars as the visual primitive rather than generic slide cards. Encode operation type consistently by color, show computational order clearly, and enclose repeated layers in a subtle labeled plate when they form one semantic block. Preserve this grammar across reference-model, block, and full-network comparisons.
...
- In code-focused framework lectures such as PyTorch, keep executable code as the principal content and place it consistently on the right. Use a compact visual on the left only to clarify the values, axes, or transformation performed by that code; never replace the code with the illustration.
- When comparing activation distributions, use histograms with consistent axes rather than encoding activation magnitude as node or ball size.
...
- Conclude a lecture by synthesizing the mechanisms already taught and showing how they can be composed for a task specification; do not use the conclusion to introduce substantial new topics.
